...
// HandleWorkflowRunQuery is the cache wrapper for the workflows run query handler
func (c *CachedDatasource) HandleWorkflowRunQuery(ctx context.Context, q *models.WorkflowRunQuery, req backend.DataQuery) (dfutil.Framer, error) {
	if value, err := c.getCache(req); err == nil {
		return value, err
	}

	f, err := c.datasource.HandleWorkflowRunQuery(ctx, q, req)
	return c.saveCache(req, f, err)
}
